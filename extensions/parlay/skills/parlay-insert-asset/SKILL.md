...
Do:
1. `insert_asset` with id `$0` into a Folder named `ParlayAssets` in Workspace (create it if missing). If Studio refuses (asset not yet approved, or not owned by this account), say exactly what it said and stop.
...
Rules: change nothing outside `Workspace.ParlayAssets`. Write no scripts. Never delete anything.
