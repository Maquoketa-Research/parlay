...
export abstract class BasePolicy implements Policy {
	...
	renderADMX(regKey: string) {
		return [
			`<policy name="${this.name}" class="Both" displayName="$(string.${this.name})" explainText="$(string.${this.name}_${this.description.nlsKey.replace(/\./g, '_')})" key="Software\\Policies\\Maquoketa-Research\\${regKey}" presentation="$(presentation.${this.name})">`,
