...
#   powershell -File build\parlay\shots.ps1 [-App <path\to\Parlay.exe>] [-Workspace <folder>] [-Out <folder>]
# Each theme gets its own temporary user-data-dir, so the captures show first-run defaults. Other Parlay windows
...
foreach ($theme in $Themes) {
  ...
  $ud = Join-Path $env:TEMP "parlay-shot-$slug"
  if (Test-Path $ud) { Remove-Item -Recurse -Force $ud }
  New-Item -ItemType Directory -Force (Join-Path $ud "User") | Out-Null
  $settings = @{ "workbench.colorTheme" = $theme; "parlay.glass" = ($theme -eq "Parlay Glass"); "window.systemColorTheme" = "auto"; "workbench.startupEditor" = "none"; "window.restoreWindows" = "none" }
