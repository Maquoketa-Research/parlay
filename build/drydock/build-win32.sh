#!/usr/bin/env bash
# Build Drydock IDE for Windows x64 from this tree: the app folder (../VSCode-win32-x64) and the user installer
# (.build/drydock/DrydockUserSetup-x64-<version>.exe). VS Code's own gulp tasks do the work; VSCodium's build
...
#
# About 15 minutes on a big machine after the first npm ci. Close a running Drydock started from the output
...
echo "== Drydock IDE ${RELEASE_VERSION} from ${BUILD_SOURCEVERSION:0:9}  $(date)"
...
mkdir -p .build/drydock
mv ".build/win32-${VSCODE_ARCH}/user-setup/VSCodeSetup.exe" ".build/drydock/DrydockUserSetup-${VSCODE_ARCH}-${RELEASE_VERSION}.exe"
ls -la .build/drydock
echo "== done  $(date)   app: $(cd .. && pwd)/VSCode-win32-${VSCODE_ARCH}/Drydock.exe"
