...
export class MeshyView implements vscode.WebviewViewProvider {
	...
	private async approveModel(job: Job) {
		...
		const form = new FormData();
		form.append("request", JSON.stringify({ assetType: "Model", displayName: job.prompt.slice(0, 50), description: `Drydock: Meshy ${job.meshyId}, painted`, creationContext: { creator } }));
...
function studioCapture(outFile: string): Promise<string | undefined> {
	return new Promise((resolve, reject) => {
		...
		(async () => {
			await call("initialize", { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "drydock-ide", version: "0.0.5" } });
