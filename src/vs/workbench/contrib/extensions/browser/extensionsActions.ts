...
export class PromptExtensionInstallFailureAction extends Action {
	...
	override async run(): Promise<void> {
		...
		if (this.error.name === ExtensionManagementErrorCode.Unsupported) {
			const productName = isWeb ? localize('Parlay for Web', "{0} for the Web", this.productService.nameLong) : this.productService.nameLong;
...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		try {
			...
			alert(localize('uninstallExtensionComplete', "Please reload Parlay to complete the uninstallation of the extension {0}.", this.extension.displayName));
		} catch (error) {
